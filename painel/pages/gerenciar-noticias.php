<?php
	if(isset($_GET['excluir'])){
		...
		$selectImagem = MySql::conectar()->prepare("SELECT capa FROM `tb_site.tours` WHERE id = ?");
		$selectImagem->execute(array($_GET['excluir']));

		$imagem = $selectImagem->fetch()['capa'];
		Painel::deleteFile($imagem);
		Painel::deletar('tb_site.tours',$idExcluir);
		Painel::redirect(INCLUDE_PATH_PAINEL.'gerenciar-noticias');
	}else if(isset($_GET['order']) && isset($_GET['id'])){
		Painel::orderItem('tb_site.tours',$_GET['order'],$_GET['id']);
	}

	$paginaAtual = isset($_GET['pagina']) ? (int)$_GET['pagina'] : 1;
	$porPagina = 22;
	
	$tours = Painel::selectAll('tb_site.tours',($paginaAtual - 1) * $porPagina,$porPagina);
...
<div class="box-content">
	...
	<table>
		...
		<tr>
			...
			<td><img style="width: 100px; height:50px" src="<?php echo INCLUDE_PATH_PAINEL ?>uploads/<?php echo $value['capa']; ?>" /></td>
	...
	<div class="paginacao">
		<?php
			$totalPaginas = ceil(count(Painel::selectAll('tb_site.tours')) / $porPagina);
