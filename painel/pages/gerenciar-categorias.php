<?php
	if(isset($_GET['excluir'])){
		...
		$tours = MySql::conectar()->prepare("SELECT * FROM `tb_site.tours` WHERE categoria_id = ?");
		$tours->execute(array($idExcluir));
		$tours = $tours->fetchAll();
		foreach ($tours as $key => $value) {
			$imgDelete = $value['capa'];
			Painel::deleteFile($imgDelete);
		}
		$tours = MySql::conectar()->prepare("DELETE FROM `tb_site.tours` WHERE categoria_id = ?");
		$tours->execute(array($idExcluir));
